...
import {
  IUserRepository,
  UserData,
} from '../../src/infrastructure/repositories/IUserRepository.js';
import {
  IPatientRepository,
  PatientData,
} from '../../src/infrastructure/repositories/IPatientRepository.js';
import {
  IDoctorRepository,
  DoctorData,
} from '../../src/infrastructure/repositories/IDoctorRepository.js';
...
describe('UserService', () => {
  ...
  beforeEach(() => {
    ...
    userService = new UserService(mockUserRepository, mockPatientRepository, mockDoctorRepository);
  ...
  describe('getUserByFiscalCode', () => {
    ...
    it('should throw error if user not found by fiscal code', async () => {
      ...
      await expect(userService.getUserByFiscalCode(fiscalCode)).rejects.toThrow('User not found');
      expect(mockUserRepository.findByFiscalCode).toHaveBeenCalledWith(fiscalCode);
    });
  ...
  describe('listUsers', () => {
    ...
    it('should return list of users with profiles', async () => {
      ...
      mockDoctorRepository.findByUserId.mockResolvedValueOnce(null).mockResolvedValueOnce({
        userId: user2Id,
        medicalLicenseNumber: 'ML123456',
        specializations: ['Cardiologia'],
      });
