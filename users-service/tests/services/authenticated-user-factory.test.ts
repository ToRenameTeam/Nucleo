...
import { Patient } from '../../src/domain/patient.js';
import { Doctor } from '../../src/domain/doctor.js';
import {
...
describe('AuthenticatedUserFactory', () => {
  ...
  beforeEach(() => {
    ...
    mockDoctor = Doctor.reconstitute(randomUUID(), 'ML123456', ['Cardiologia']);
  });
  ...
  describe('create', () => {
    ...
    it('should throw error when only doctor profile is provided (doctor-only not supported)', () => {
      expect(() => AuthenticatedUserFactory.create(mockUser, null, mockDoctor)).toThrow(
        'Doctor-only profile not supported. Doctors must also be patients.'
      );
    });

    it('should throw error when no profiles are provided', () => {
      expect(() => AuthenticatedUserFactory.create(mockUser, null, null)).toThrow(
...
describe('DoctorPatientUser', () => {
  ...
  beforeEach(() => {
    mockUser = { userId: randomUUID() } as unknown as User;
    mockPatient = Patient.reconstitute(randomUUID());
    mockDoctor = Doctor.reconstitute(randomUUID(), 'ML123456', ['Cardiologia', 'Medicina Interna']);
  });
