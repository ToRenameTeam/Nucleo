...
class DeleteEventsConsumer(
    private val cleanupRepository: ExposedAppointmentRepository,
    private val bootstrapServers: String,
    private val clientId: String,
    private val groupId: String,
    private val userDeletedTopic: String,
    private val facilityDeletedTopic: String,
    ...
    private fun isEnabled(): Boolean {
        return bootstrapServers.isNotBlank() &&
            groupId.isNotBlank() &&
            listOf(userDeletedTopic, facilityDeletedTopic, serviceTypeDeletedTopic).all {
                it.isNotBlank()
            }
    }

    private fun runConsumerLoop() {
        val topics = listOf(userDeletedTopic, facilityDeletedTopic, serviceTypeDeletedTopic)
    ...
    private fun handleRecord(topic: String, payload: String) {
        try {
            when (topic) {
                ...
                facilityDeletedTopic -> handleFacilityDeleted(payload)
    ...
    private fun handleUserDeleted(payload: String) {
        ...
        runBlocking {
            val stats = cleanupRepository.cleanupByDeletedUser(userId)
            logger.info(
                "Processed user-deleted event for {}. deletedAppointments={}, deletedAvailabilities={}",
                userId,
                stats.deletedAppointments,
                stats.deletedAvailabilities
            )
        }
    }

    private fun handleFacilityDeleted(payload: String) {
        val facilityId = extractField(payload, "id")

        runBlocking {
            val stats = cleanupRepository.cleanupByDeletedFacility(facilityId)
            logger.info(
                "Processed facility-deleted event for {}. deletedAppointments={}, deletedAvailabilities={}",
                facilityId,
                stats.deletedAppointments,
                stats.deletedAvailabilities
            )
    ...
    private fun handleServiceTypeDeleted(payload: String) {
        ...
        runBlocking {
            val stats = cleanupRepository.cleanupByDeletedServiceType(serviceTypeId)
            logger.info(
                "Processed service-type-deleted event for {}. deletedAppointments={}, deletedAvailabilities={}",
                serviceTypeId,
                stats.deletedAppointments,
                stats.deletedAvailabilities
            )
