    ...
    DescribeSpec({
        ...
        describe("GET /api/v1/patients/{patientId}/documents/{documentId}") {
            it("should return 404 for non-existent document") {
                configuredTestApplication { client ->
                    val response =
                        client.get("/api/v1/patients/patient-1/documents/non-existent-id")
        ...
        describe("POST /api/v1/patients/{patientId}/documents") {
            ...
            describe("Service Prescription") {
                it("should create service prescription with valid data") {
                    configuredTestApplication { client ->
                        ...
                        created.id shouldNotBe null
                        created.patientId shouldBe patientId
                        created.serviceId shouldBe
                            (request as CreateServicePrescriptionRequest).serviceId
            ...
            describe("Report") {
                it("should create report linked to existing service prescription") {
                    configuredTestApplication { client ->
                        ...
                        val reportRequest: CreateDocumentRequest =
                            DocumentFixtures.reportRequest(servicePrescriptionId = prescription.id)
                        val response =
                            client.post("/api/v1/patients/$patientId/documents") {
                                contentType(ContentType.Application.Json)
                ...
                it("should return 400 when trying to link report to medicine prescription") {
                    configuredTestApplication { client ->
                        ...
                        val medicinePrescription =
                            medicineResponse.body<MedicinePrescriptionResponse>()
            ...
            describe("Validation") {
                it("should return 400 for invalid JSON") {
                    configuredTestApplication { client ->
                        val response =
                            client.post("/api/v1/patients/patient-1/documents") {
                                contentType(ContentType.Application.Json)
        ...
        describe("DELETE /api/v1/patients/{patientId}/documents/{documentId}") {
            it("should delete existing document") {
                configuredTestApplication { client ->
                    ...
                    // When: deleting it
                    val deleteResponse =
                        client.delete("/api/v1/patients/$patientId/documents/${created.id}")
                    ...
                    val getResponse =
                        client.get("/api/v1/patients/$patientId/documents/${created.id}")
            ...
            it("should return 404 when deleting non-existent document") {
                configuredTestApplication { client ->
                    val response =
                        client.delete("/api/v1/patients/patient-1/documents/non-existent")
        ...
        describe("PUT /api/v1/patients/{patientId}/documents/{documentId}/report") {
            it("should update report findings") {
                configuredTestApplication { client ->
                    ...
                    val response =
                        client.put("/api/v1/patients/$patientId/documents/${report.id}/report") {
                            contentType(ContentType.Application.Json)
            ...
            it("should return 400 when trying to update non-report document") {
                configuredTestApplication { client ->
                    ...
                    val response =
                        client.put(
                            "/api/v1/patients/$patientId/documents/${prescription.id}/report"
                        ) {
                            contentType(ContentType.Application.Json)
                            setBody(updateRequest)
                        }
            ...
            it("should return 404 when updating non-existent report") {
                configuredTestApplication { client ->
                    val updateRequest = UpdateReportRequest(findings = "New findings")
                    val response =
                        client.put("/api/v1/patients/patient-1/documents/non-existent/report") {
                            contentType(ContentType.Application.Json)
                            setBody(updateRequest)
                        }
            ...
            it("should allow partial updates") {
                configuredTestApplication { client ->
                    ...
                    val updateRequest =
                        UpdateReportRequest(recommendations = "Updated recommendations only")
                    val response =
                        client.put("/api/v1/patients/$patientId/documents/${report.id}/report") {
                            contentType(ContentType.Application.Json)
                            setBody(updateRequest)
