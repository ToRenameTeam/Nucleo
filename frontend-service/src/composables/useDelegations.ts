...
export function useDelegations() {
  ...
  const loadDelegations = async (userId: string, type: 'received' | 'sent'): Promise<void> => {
    ...
    try {
      ...
      const items = await Promise.all(
        response.delegations.map(async (delegation) => {
          const targetUserId =
            type === 'received' ? delegation.delegatorUserId : delegation.delegatingUserId;
  ...
  const acceptDelegation = async (delegationId: string, userId: string): Promise<boolean> => {
    try {
  ...
  const declineDelegation = async (delegationId: string, userId: string): Promise<boolean> => {
    try {
  ...
  const removeDelegation = async (delegationId: string, userId: string): Promise<boolean> => {
    try {
  ...
  const countDelegations = async (userId: string): Promise<{ received: number; sent: number }> => {
    try {
